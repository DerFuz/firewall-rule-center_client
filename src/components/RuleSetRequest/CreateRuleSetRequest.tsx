            ...
            if (results.errors.length !== 0) {
              ...
              const newResults = results.data.map(element => {
                return {
                  ...
                  'source_ip_orig': element.source_ip_orig ? element.source_ip_orig : '',
                  'source_ip_nat': element.source_ip_nat ? element.source_ip_nat : '',
                  'source_port': element.source_port ? element.source_port : 0,
                  'destination_name': element.destination_name,
                  'destination_ip_orig': element.destination_ip_orig ? element.destination_ip_orig : '',
                  'destination_ip_nat': element.destination_ip_nat ? element.destination_ip_nat : '',
                  'destination_port': element.destination_port ? element.destination_port : 0,
                  'requester': element.requester,
                  'ticket': element.ticket ? element.ticket : '',
                  'rule_set_request': 0,
                  'notes': element.notes ? element.notes : '',
                  'firewalls': element.firewalls ? element.firewalls.toString().split(',').map(firewall => {
                    return { 'hostname': firewall }
                  }) : [],
                  'status': RuleStatusEnum.Req,
                  'is_deleted': false,
                }
